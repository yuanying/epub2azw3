...
- MaxFileSize: int — 最大ファイルサイズ（デフォルト256KB、KF8ターゲット）
- MinJPEGQuality: int — JPEG最低品質（デフォルト60）
- CoverJPEGQuality: int — カバー画像JPEG品質（デフォルト90）
- MaxPixels: int — デコード前の総ピクセル数上限（デフォルト100メガピクセル）
...
- Warning: string — パススルーやサイズ超過時の警告メッセージ（空なら正常）

## 実装ガイドライン
- `github.com/disintegration/imaging` パッケージを使用
- 画像処理パイプライン: DecodeConfig（サイズガード） → デコード → リサイズ → 形式変換 → 圧縮 → サイズ検証
- サイズ超過時の自動品質調整: 品質を5刻みで下げて再圧縮（最低60まで）
- カバー画像は別のパラメータセットで処理（リサイズスキップ、品質90以上）
- デコード失敗・巨大画像・サイズ超過は `Warning` フィールドで報告し、`error` は真のエンコードエラーのみに使用
- デコード前に `DecodeConfig` で総ピクセル数をチェックし、上限超過時はパススルー（OOM防止）
- SVG除外はMIME正規化（大文字・パラメータ付き対応）で判定
